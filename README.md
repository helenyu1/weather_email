...
仅在工作日的 17:30 发送邮件，内容包含上海当天 18:00 的小时天气和明天的天气预报。
...
项目内已包含 `.github/workflows/daily-weather-email.yml`，默认每天 `17:30` 上海时间运行；脚本内部会在周末自动跳过。

GitHub Actions 的 cron 使用 UTC，所以 workflow 中配置的是 `09:30 UTC`。
